...
class AuthPage extends Page {
  ...
  AuthPage({Key key}) : super(key: null);
  //루트페이지 생성
      ...
      onPressed: () async {
        if (_formKey.currentState.validate()) {
          if (isRegister) {
            try {
              //create user
              //파이어베이스에서 UserCredential을 통해 유저의 정보를 가져와 연동을 시켜준다  createUserWithEmailAndPassword로 계정생성을 진행
              ...
              if (e.code == 'email-already-in-use') {
                SnackBar snackBar =
                    SnackBar(content: Text('해당 이메일은 이미 사용중입니다.'));
                ...
                SnackBar snackBar =
                    SnackBar(content: Text('operation-not-allowed'));
                _scaffoldKey.currentState.showSnackBar(snackBar);
              } else if (e.code == 'weak-password') {
                SnackBar snackBar = SnackBar(content: Text('weak-password'));
                _scaffoldKey.currentState.showSnackBar(snackBar);
              }
            }
          } else {
